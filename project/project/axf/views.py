...
def home(request):
    ...
    return render(request,
                  'axf/home.html', {'title':'主页', 'wheelsList':wheelList,
...
def market(request,categoryid,cid,sortid):
    typename = FoodTypes.objects.all()
    ...
    if sortid == '1':
        ...
        productList.order_by('price')
    elif sortid == '3':
        productList.order_by('-price')

    group = typename.get(typeid = categoryid)
    childList = []
    ...
    return render(request, 'axf/market.html', {'title':'闪送超市',
                        'leftSlider':typename, 'productList':productList,
                                               'childList':childList, 'categoryid':categoryid,
                                               'cid':cid,})
...
#                 c.productnum += 1
#                 c.producprice = '%.2f'(float(product.price) * (c.productnum))
#                 c.save()
#
